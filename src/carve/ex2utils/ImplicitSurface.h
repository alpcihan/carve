...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double t_x = _x.x()-m_center.x();
		double t_y = _x.y()-m_center.y();
		double t_z = _x.z()-m_center.z();

		return pow(t_x,2) + pow(t_y,2)  + pow(t_z,2) - pow(m_radius,2) ;
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		return     pow( pow(t_x,2) + pow(t_y,2) + pow(t_z,2) + pow(m_radius,2) - pow(m_a,2) ,2)    -4*pow(m_radius,2) *(pow(t_x,2)+ pow(t_y,2));
	}
...
{
public:
	Hoppe(std::vector<bool>& space)
	{
		m_pointcloud.ReadFromSpace(space);
	}
...
#define phix(i,_x) EvalBasis((m_funcSamp.m_pos[i]-_x).norm())
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for (unsigned int i = 0; i<m_numCenters;i++) {
			result += m_coefficents[i]*phix(i,_x);
		}
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for (unsigned int i = 0; i<2*m_numCenters;i++) {
			...
			A(i,j) = m_funcSamp.m_pos[i].x(); j++; A(i,j) = m_funcSamp.m_pos[i].y(); j++; A(i,j) = m_funcSamp.m_pos[i].z(); j++; A(i,j) = 1; 
		}

		CRV_INFO("CONFIGURING SYSTEM")
